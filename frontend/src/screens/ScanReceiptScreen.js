...
import {
  View, Text, TouchableOpacity, StyleSheet, Alert, ActivityIndicator, ScrollView, TextInput
} from 'react-native';
  ...
  if (loading) {
    return (
      <View style={styles.centred}>
        ...
        <Text style={styles.loadingText}>{step === 'pick' ? 'Processing receipt…' : 'Saving…'}</Text>
      </View>
    );
  }
    ...
    return (
      <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}>
        ...
        <Text style={styles.hint}>
          OCR confidence: {scanResult?.confidence}%. Edit any incorrect items below.
        </Text>
...
const styles = StyleSheet.create({
  ...
  secondaryBtn: {
    ...
    paddingHorizontal: 32, paddingVertical: 16, borderRadius: 28, width: '100%', alignItems: 'center',
  },
  ...
  itemPrice: {
    borderWidth: 1, borderColor: '#ddd', borderRadius: 8, paddingHorizontal: 10,
    paddingVertical: 6, width: 80, fontSize: 14, marginRight: 8,
  },
